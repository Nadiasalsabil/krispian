...
<?php
 ...
 $sql="select * from `$tbdatalatih`  order by `id_dataset` asc limit 0,2";		//		limit 0,10			
...
echo"</tr>";
 ...
 // $log=log($jumdoc+1,10)/$jumada; 
  $log=log(($jumdoc)/$jumada,10); 
 $log=abs($log);
 echo"<td>$jumada</td>";//idf
 echo"<td>log($jumdoc/$jumada)=$log";
...
echo"<h1>Bobot :".$PROK[$i-1]."</h1>";
 //$sql="UPDATE `$tbpengajuan` set status= '".$PROK[$i-1]."',catatan='$catatan',rekapitulasi='$rekapitulasi' where `id_datauji`='$id_datauji'";
//$up=process($conn,$sql);
