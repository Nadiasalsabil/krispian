...
<?php
$edit="SELECT * FROM  `tbl_datatesting` WHERE id_datatesting='$_GET[id]'";
...
?>
 ...
 <form action="" method="post" enctype="multipart/form-data">
	<div class="form-group row">
	    <input type="hidden" class="form-control" id="id" name="id" value=<?php echo $data['id_datatesting']; ?>>
	</div>
     ...
     <div class="form-group row">
        <label class="col-sm-3 col-form-label">Nilai Sentimen</label>
            <div class="col-sm-9">
                ...
                <option value="<?php echo $data['sentimen'];?>"><?php 
                    if($data['sentimen']==1){echo "Positif";}
                    elseif($data['sentimen']==-1){echo "Negatif";}
                    else{echo "Netral";} ?></option>
...
if (isset($_POST['simpan'])) 
{
    ...
    $bikinos = $initos->createStemmer();
	 ...
	 $update     = "UPDATE tbl_datatesting SET tweet='$judul',sentimen='$sentimen',normalisasi='$stemming' WHERE id_datatesting='$id'";
   $query = mysqli_query($conn, $update) or die(mysqli_error($conn));
	...
	echo "<script>location='index.php?mnu=datatesting';</script>";
    }
    else{
